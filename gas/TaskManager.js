...
function isTaskCellHighlightActive_(taskValue) {
  const taskLower = String(taskValue || "").trim().toLowerCase();

  return taskLower === TASK_CONFIG.STATUS.RESERVE || taskLower === "reserve_to_wait";
}

function isQueCellHighlightActive_(queValue) {
  const queLower = String(queValue || "").trim().toLowerCase();
  return queLower === TASK_CONFIG.QUE_FLAG.ON;
}
...
function getTaskItemsByTaskAndQue(task, queFlag) {
  ...
  return rows.filter(item => item.task === task && item.que === queFlag);
}

function getReservedTaskItemsReadyForQueueing() {
  const now = new Date();
  const targets = getTaskItemsByTaskAndQue(TASK_CONFIG.STATUS.RESERVE, TASK_CONFIG.QUE_FLAG.OFF)
...
function reserveTaskByTaskItems_(taskItems, target) {
  ...
  const candidate = taskItems
    ...
    .find(item => item.que === TASK_CONFIG.QUE_FLAG.OFF && item.task === TASK_CONFIG.STATUS.NONE);
...
function setTaskQueuedOnById_(taskId) {
  if (!taskId) {
    return false;
  }
  ...
  return updateTaskById_(taskId, {
    que: TASK_CONFIG.QUE_FLAG.ON,
    queTime: now
  });
}

function setTaskQueuedOffById_(taskId) {
  if (!taskId) {
    return false;
  }
...
function setTaskQueuedOnByKey(key) {
  if (!key) {
    return false;
  }
...
function setTaskQueuedOnByKeyAndTarget(key, target) {
  if (!key) {
    return false;
  }
  ...
  const candidate = rows
    .filter(item => (
      ...
      item.task === TASK_CONFIG.STATUS.RESERVE &&
      item.que === TASK_CONFIG.QUE_FLAG.OFF
    ))
  ...
  return setTaskQueuedOnById_(candidate.id);
}

function setTaskQueuedOffByKey(key) {
  if (!key) {
    return false;
  }
...
function clearTimedOutQueuedTasks_(thresholdMinutes) {
  ...
  rows.forEach(item => {
    if (item.que !== TASK_CONFIG.QUE_FLAG.ON) {
      return;
    }
